...
const TransactionTable = () => {
  ...
  const [appliedPromotion, setAppliedPromotion] = useState(null); // ✅ Lưu khuyến mại đã áp dụng

  const [point, setPoint] = useState();

  // ✅ Hàm áp dụng khuyến mại
  const applyPromotion = (item, promotionType) => {
    const promotion = {
      type: promotionType,
      item: item,
      originalPoints: item.diem_tngo,
    };

    // Tính toán chiết khấu dựa trên loại khuyến mại
    if (promotionType === "first_buy") {
      promotion.discount = 50; // 50% giảm
      promotion.discountedPoints = Math.floor(item.diem_tngo * 0.5);
    } else if (promotionType === "vip") {
      promotion.discount = 30; // 30% giảm
      promotion.discountedPoints = Math.floor(item.diem_tngo * 0.7);
    } else if (promotionType === "seasonal") {
      promotion.discount = 20; // 20% giảm
      promotion.discountedPoints = Math.floor(item.diem_tngo * 0.8);
    }
  ...
  useEffect(() => {
    if (id) {
      ...
      setPostUrl(`${apiUrl}/api/auth/payment-ticket?ve_id=${id}`);
    }
  }, [id]);
  ...
  const {
    ...
    process.env.NEXT_PUBLIC_API_URL
      ? `${process.env.NEXT_PUBLIC_API_URL}/api/auth/tickets/getTickets?page=1`
