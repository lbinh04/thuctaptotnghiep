...
const Rifd = () => {
  ...
  const [userCard, setUserCard] = useState(null); // Track user's current card
  ...
  const { data, loading, error } = useFetchGetData(
    process.env.NEXT_PUBLIC_API_URL
      ? `${process.env.NEXT_PUBLIC_API_URL}/api/auth/card/getCards?page=1`
  ...
  useEffect(() => {
    if (id) {
      ...
      setPostUrl(`${apiUrl}/api/auth/payment-card?theId=${id}`);
    }
  }, [id]);
  ...
  useEffect(() => {
    if (data) {
      setCards(data?.cards);
      setUserCard(data?.userCard || null); // Get user's current card if exists
    }
  }, [data]);
          ...
          <div className="flex flex-wrap justify-between mb-16 mt-5">
            ...
            {isOpen && cards?.length > 0 ? (
              cards.map((card, index) => {
                // Check if this card can be purchased
                const isRideUp = card.loai_the === "RideUp";
                    ...
                    <div className="w-[31%] flex flex-col gap-3 justify-between">
                      ...
                      <div className="text-[17px]">
                        <p className="">
                          ...
                          {isRideUp && ", miễn phí mở thẻ lần đầu"}
                        </p>
                        <p className="">
